...
typedef void(^XYPickerViewDXYZBlock)(id x,id y,id z);
    ...
    [self.rightBtn bk_whenTapped:^{
        switch (self.type) {
            ...
            case PickerTypeDate:
            {
                ...
                [dateFormatter setDateFormat:@"yyyy-MM-dd"]; //大写的 H为24小时制，小写h为12小时制
            ...
            case PickerTypeModelNeedIndex:
            {
                ...
                if (self.pickerViewXYZBlock) {
                    [self getFinalValue];
                    self.pickerViewXYZBlock(self.finalValue,[NSNumber numberWithInteger:self.currentCompent],_indexPath);
                }
            }
                break;
            default:
