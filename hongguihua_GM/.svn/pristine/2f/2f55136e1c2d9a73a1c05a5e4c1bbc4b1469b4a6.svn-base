...
+(void)login:(UMengLoginType)loginType vc:(UIViewController *)vc resBlock:(void(^)(UMSocialAccountEntity *snsAccount,UMengLoginType loginType))resBlock{
    ...
    }else if (loginType == UMengLoginTypeTaoBao){//淘宝
        UMSocialSnsPlatform *snsPlatform = [UMSocialSnsPlatformManager getSocialPlatformWithName:UMShareToAlipaySession];
        
        snsPlatform.loginClickHandler(vc,[UMSocialControllerService defaultControllerService],YES,^(UMSocialResponseEntity *response){
            
            if (response.responseCode == UMSResponseCodeSuccess) {
                
                UMSocialAccountEntity *snsAccount = [[UMSocialAccountManager socialAccountDictionary]valueForKey:UMShareToAlipaySession];
